...
module Header =
    let private classes = {|
        navbar = "dark:md:bg-gray-900 fixed w-full z-100  top-0 start-0   bg-transparent"
        ...
        itemActive = "text-white bg-blue-700 rounded-sm md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500"
        itemInactive =
            "text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-green-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700"
    |}
    ...
    let Header (scrolled: Accessor<float>) : HtmlElement =
        ...
        let calculateWidthLogo () =
            ...
            let ratioscrolled = maxRes - int(float(maxRes - minRes) * min (scrolled() / wH) 1.)
        ...
        nav(class' = classes.navbar) {
            div(class' = classes.navbarContainer) {
                ...
                div(class' = classes.linkListContainer, id = "navbar-sticky")
                    .classList(
                        {|
                            ``not-md:opacity-0 not-md:pointer-events-none`` = not(triggeredMenu())
                            ``not-md:opacity-75`` = triggeredMenu()
                        |}
                    ) {
